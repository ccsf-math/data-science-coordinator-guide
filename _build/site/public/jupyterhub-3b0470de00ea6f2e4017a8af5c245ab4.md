...
flowchart TD;
    ...
    B --> C{Is user authenticated with JupyterHub?}

    C -- No --> D[Redirect to login (e.g., CILogon/Google)]
    D --> E[User completes authentication]
    E --> F[Access granted to JupyterHub environment]

    C -- Yes --> F

    F --> G[nbgitpuller fetches repository content from GitHub]
    G --> H[nbgitpuller pulls files into user's Jupyter environment]
