...
```mermaid
flowchart TD
    A[User clicks nbgitpuller link] --> B{Is user authenticated?}
    ...
    K --> L[Pull files into user environment]
    L --> M[Open notebook or landing page (optional)]

    %% Subgraphs (for visual grouping only — don't reuse nodes inside them)
    subgraph Canvas_or_Website
        A
    end
